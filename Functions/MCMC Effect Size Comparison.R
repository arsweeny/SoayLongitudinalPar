# MCMC effect size comparison ####

MCMCFxCompInt<-function(model, vars, numeric = T){
  ...
  if(!is.null(vars)){
    for(x in 1:length(vars)){
      ...
      Factors <- vars[x]
      Selected <- unlist(str_split(Factors, "[:]"))[2]
      Columns<-Solutions %>% as.data.frame() %>%  
      ...
      if(dim(Columns)[2]>1){
        ...
        Names <-colnames(Columns) %>% as.data.frame()
        NamesNew <- c(0, Names %>% slice(-1) %>% separate(1, into=c("Level", "Term"), sep="[:]") %>% pull(Level))
        ...
        dimnames(df1) <- dimnames(df1l) <- dimnames(df1u) <- dimnames(df1p) <-list(NamesNew, NamesNew)
        ...
        FactorCompList[[x]] <- list(Mean=df1, Lower=df1l, Upper=df1u, pMCMC=df1p)
          names(FactorCompList)[x]<-Selected
        
      }else{
        Column<-Solutions %>% as.data.frame() %>%  
          select(contains(Selected)) %>% 
          select(-Selected)
    ...
    if(numeric == F){
      ...
      if(length(FactorCompList[[x]])>0){
        ...
        dimnames(FactorCompList[[x]])<-list(NamesNew, NamesNew)
      }
    }
    ...
    for(x in 1:length(Factors)){
      ...
      if(dim(Columns)[2]>1){
        ...
        dim(df1)<-rep(ncol(Columns),2)
        dimnames(df1)<-list(NamesNew, NamesNew)
        colnames(df1)[1] <- colnames(df1l)[1] <- colnames(df1u)[1] <- colnames(df1p)[1] <- "Intercept" 
        rownames(df1)[1] <- rownames(df1l)[1] <- rownames(df1u)[1] <- rownames(df1p)[1] <- "Intercept" 
        FactorCompList[[x]]<- list(Mean = df1, Lower = df1l, Upper = df1u, pMCMC = df1p)
...
# Effect size comparison non - interactions 
